...
let heroes = {
    batman: {
      ...
      img: "../img/1.jpg",
      rating: JSON.parse(localStorage.getItem("batman rating")) || "",
      more:
    ...
    superman: {
      ...
      img: "../img/2.jpg",
      rating: JSON.parse(localStorage.getItem("superman rating")) || "",
      more:
  ...
  for (let h in heroes) {
    out += "<div class='hero " + heroes[h].id + "'>";
  
    out += '<img src="' + heroes[h].img + '"><br>';
    out += heroes[h].name + "<br>";
    out += "Вселенная: " + heroes[h].universe + "<br>";
    out += "Альтер эго: " + heroes[h].alterEgo + "<br>";
    out += "Род деятельности: " + heroes[h].occupation + "<br>";
    out += "Друзья: " + heroes[h].friends + "<br>";
    out += "Суперсила: " + heroes[h].superpower + "<br>";
    out += "<label> Рейтинг <span>" + heroes[h].rating + "</span>:";
  
    console.log(heroes[h].rating);
    if (heroes[h].rating === "1") {
      out += `<input type="radio" name="${heroes[h].id}" value="1" checked>`;
    } else {
      out += `<input type="radio" name="${heroes[h].id}" value="1">`;
    }
  
    if (heroes[h].rating === "2") {
      out += `<input type="radio" name="${heroes[h].id}" value="2" checked>`;
    } else {
      out += `<input type="radio" name="${heroes[h].id}" value="2">`;
    }
  
    if (heroes[h].rating === "3") {
      out += `<input type="radio" name="${heroes[h].id}" value="3" checked>`;
    } else {
      out += `<input type="radio" name="${heroes[h].id}" value="3">`;
    }
  
    if (heroes[h].rating === "4") {
      out += `<input type="radio" name="${heroes[h].id}" value="4" checked>`;
    } else {
      out += `<input type="radio" name="${heroes[h].id}" value="4">`;
    }
  
    if (heroes[h].rating === "5") {
      out += `<input type="radio" name="${heroes[h].id}" value="5" checked>`;
    } else {
      out += `<input type="radio" name="${heroes[h].id}" value="5">`;
    }
  
    out += "Подробнее: " + heroes[h].more + "<br>";
    out += "</div>";
  }
  document.querySelector(".out").innerHTML = out;
  
  // все радиокнопки на странице!
  ...
  for (let i = 0; i < ratings.length; i++) {
    ...
    ratings[i].addEventListener("click", function (event) {
      ...
      // сохраняем в localStorage записываем выбранный рейтинг
      // "batman rating": 3
